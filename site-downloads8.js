...
async function refreshDownloadCounts() {
  ...
  try {
    ...
    for (let page = 1; page <= 10; page += 1) {
      const response = await fetch(`https://api.github.com/repos/vanyima/mashangxiaban-releases/releases?per_page=100&page=${page}`, {
    ...
    counters.forEach((counter) => {
      ...
      counter.textContent = `累计 ${new Intl.NumberFormat('zh-CN').format(count)} 次下载`;
    });
  } catch {
    counters.forEach((counter) => counter.textContent = '累计下载次数暂不可用');
  }
}
